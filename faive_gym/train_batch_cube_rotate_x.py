...
max_iterations = 500
no_envs = 8000
wandb_group = "hand_b_cube_rotate_x"
base_command = f"python train.py task=hand_BP0_cube_rotate_x max_iterations={max_iterations} num_envs={no_envs} wandb_group={wandb_group} wandb_activate=True capture_video=True force_render=False test=False"



hand_angles = {"20":[ 0.6963642, -0.1227878, 0.6963642, -0.1227878 ],"25":[ 0.6903455, -0.1530459, 0.6903455, -0.1530459 ]} # not yet implemented
rolling_directions = [-1] #-1 is easier, 1 is harder
...
x_rotvel_rewards = [0.18, 0.15, 0.12,0.1] #,0.15,0.05]# [0.01,0.02,0.05,0.1,0.001,0.005] # make sure these are floats
            ...
            for drop_penalty in drop_penalities:
                for x_rotvel_reward in x_rotvel_rewards:
                    for rolling_direction in rolling_directions:
                        wandb_name = f"cube_rolling_x_direction_{rolling_direction}_x_rotvel_{x_rotvel_reward}_drop_{drop_penalty}_hand_angle_25_seed_{seed}_relative_{relative_ctrl}_action_{action_panelty}"
